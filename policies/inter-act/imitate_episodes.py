...
from inter_act_policy import InterACTPolicy
...
def main(args):
    ...
    ddp = local_rank >= 0
    if ddp:
        # 30-min collective timeout (default 10 min): with 4 concurrent DDP jobs sharing the NAS,
        # a worker's read can stall long enough that the peer's barrier/all-reduce would otherwise
        # trip the NCCL watchdog and abort the rank (observed on T1/T4).
        from datetime import timedelta
        dist.init_process_group(backend="nccl", timeout=timedelta(minutes=30))
        torch.cuda.set_device(local_rank)
    # global rank for the single writer (LOCAL_RANK is per-node, so it would have a rank-0 on every
    # node under multi-node torchrun); set_device above still uses the per-node local_rank.
    ...
    if policy_class in ("ACT", "InterACT"):
        ...
        if policy_class == "InterACT":
            policy_config.update({
                ...
                "n_pre_decoder_layers": args["n_pre_decoder_layers"],
                "n_post_decoder_layers": args["n_post_decoder_layers"],
                "n_sync_decoder_layers": args["n_sync_decoder_layers"],
            })
    ...
    for rollout_id in range(num_rollouts):
        ...
        with torch.inference_mode():
            for t in range(max_timesteps):
                ...
                ### query policy
                if config["policy_class"] in ("ACT", "InterACT"):
...
def forward_pass(data, policy):
    ...
    # On a single GPU .mean() of a 0-dim scalar is a no-op, so this is safe either way.
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    if ddp:
        # find_unused_parameters=True: the ACT policy has a few params that don't take grad every
        # step (e.g. unused CNNMLP head paths) -- without this DDP's reducer errors on the missing
        # grad. Small extra pass, negligible for an 84M model.
