...
import numpy as np
import torch
from .models import build_ACT_model, build_CNNMLP_model, build_INTERACT_model
...
def build_INTERACT_model_and_optimizer(args_override, RoboTwin_Config=None):
    if RoboTwin_Config is None:
        parser = argparse.ArgumentParser("InterACT training and evaluation script", parents=[get_args_parser()])
        args = parser.parse_args()
        for k, v in args_override.items():
            setattr(args, k, v)
    else:
    ...
    print("build_INTERACT_model_and_optimizer", args)
    model = build_INTERACT_model(args)
    device = torch.device(getattr(args, "device", "cuda" if torch.cuda.is_available() else "cpu"))
    model.to(device)

    param_dicts = [
        {
            "params": [p for n, p in model.named_parameters() if "backbones" not in n and "backbone" not in n and p.requires_grad]
        },
        {
            "params": [p for n, p in model.named_parameters() if ("backbones" in n or "backbone" in n) and p.requires_grad],
            "lr": args.lr_backbone,
        },
    ]
    optimizer = torch.optim.AdamW(param_dicts, lr=args.lr, weight_decay=args.weight_decay)
